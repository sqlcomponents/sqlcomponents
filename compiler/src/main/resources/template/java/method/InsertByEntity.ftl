            ...
            public final ${name} returning() throws SQLException  {
                     ...
                     PreparedStatement preparedStatement = dbConnection.prepareStatement(query<#if table.hasAutoGeneratedPrimaryKey == true>, Statement.RETURN_GENERATED_KEYS</#if>))
                {
                    ...
                    if( preparedStatement.executeUpdate() == 1 ) {
                      ...
                      if (res.next()) {
                          inserted${name} =  insertStatement.${name?uncap_first}Store${orm.daoSuffix}.find(${getGeneratedPrimaryKeysFromRS()}).get();
                      }
                      <#else>
                      inserted${name} =  insertStatement.${name?uncap_first}Store${orm.daoSuffix}.find(${getPrimaryKeysFromModel(name?uncap_first)}).get();
                      </#if>
                  }
                }
                    ...
                    if ( insertedCounts != null ) {
                        ...
                        for (int i = 0; i < insertedCounts.length; i++) {
                          ...
                          if (insertedRows == 1 && res.next()) {
                            insertedList.add(insertStatement.${name?uncap_first}Store${orm.daoSuffix}.find(${getGeneratedPrimaryKeysFromRS()}).get()) ;
                          }
                          <#else>
                          if (insertedRows == 1) {
                            insertedList.add(insertStatement.${name?uncap_first}Store${orm.daoSuffix}.find(${getPrimaryKeysFromModel("listOf${name}.get(i)")}).get());
                          }
                          </#if>
                          else {
