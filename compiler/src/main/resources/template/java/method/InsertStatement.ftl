            ...
            public ${name} returning() throws <@throwsblock/>  {
                ...
                prepare(sqlBuilder,${name?uncap_first});
                    ...
                    if( sqlBuilder.prepare().executeUpdate() == 1 ) {
                                          ...
                                          if (res.next()) {
                                              inserted${name} =  ${name}Store.this.select(${getGeneratedPrimaryKeysFromRS()}).get();
                                          }
                                          <#else>
                                          inserted${name} =  ${name}Store.this.select(${getPrimaryKeysFromModel(name?uncap_first)}).get();
                                          </#if>
                                      }
                    </#if>
              ...
              .query(resultSet -> rowMapperForReturning(resultSet, ${name?uncap_first}s.get(atomicInteger.getAndIncrement()))).list(); 
                    ...
                    if( sqlBuilder.prepare().executeUpdate() == 1 ) {
                        inserted${name}s = new ArrayList<>(${name?uncap_first}s.size());
                                          ...
                                          if (res.next()) {
                                              inserted${name}s.add(${name}Store.this.select(${getGeneratedPrimaryKeysFromRS()}).get());
                                          }
                                          <#else>
                                          for(${name} ${name?uncap_first}:${name?uncap_first}s) {
                                          inserted${name}s.add(${name}Store.this.select(${getPrimaryKeysFromModel(name?uncap_first)}).get());
                                          }
                                          </#if>
                                      }
                    </#if>
