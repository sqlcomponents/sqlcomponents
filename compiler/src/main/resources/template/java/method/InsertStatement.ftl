...
<#if table.tableType == 'TABLE' >
    ...
    public final class InsertStatement {
        ...
        private void prepare(final SqlBuilder.PreparedSqlBuilder sqlBuilder,final ${name} ${name?uncap_first}) {
            <#assign index=0>
        ...
        private void prepare(final SqlBuilder.PreparedSqlBuilder sqlBuilder,final List<${name}> ${name?uncap_first}s) {
            ...
            public ${name} returning(final DataSource dataSource) throws <@throwsblock/>  {
                ...
                prepare(sqlBuilder,${name?uncap_first});
                    ...
                    <#else>
                        ...
                        ${name} created = sqlBuilder
                                                .queryGeneratedKeys(rs -> ${name}Store.this.rowMapper(rs))
                                                .execute(dataSource);


                        
                        inserted${name} =  ${name}Store.this.select(dataSource,${getPrimaryKeysFromModel("created")}).get();
      ...
      query += <@compress single_line=true>" <@returning/>"</@compress>;
              ...
              .queryForList(resultSet -> rowMapperForReturning(resultSet, ${name?uncap_first}s.get(atomicInteger.getAndIncrement()))).execute(dataSource); 
                    ...
                    <#else>
                        ...
                        <#if table.hasAutoGeneratedPrimaryKey == true>
                        List<${name}> createdItems = sqlBuilder
                                                .queryGeneratedKeysAsList(rs -> ${name}Store.this.rowMapper(rs))
                                                .execute(dataSource);

                            for (${name} created : createdItems) {
                                inserted${name}s.add(${name}Store.this.select(dataSource,${getPrimaryKeysFromModel("created")}).get());
                            }
                        <#else>
	...
	<#assign a=addImportStatement(beanPackage+"."+name)><#assign a=addImportStatement("java.util.Arrays")>
</#if>