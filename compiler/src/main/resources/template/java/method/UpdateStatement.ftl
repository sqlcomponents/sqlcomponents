        ...
        public static final class SetByPKClause  {
                ...
                public final ${name} returning() throws SQLException  {
                        ...
                        PreparedStatement preparedStatement = dbConnection.prepareStatement(query<#if table.hasAutoGeneratedPrimaryKey == true>, Statement.RETURN_GENERATED_KEYS</#if>))
                    {
                        ...
                        if (res.next()) {
                            updated${name} =  updateStatement.${name?uncap_first}Store${orm.daoSuffix}.select(${getGeneratedPrimaryKeysFromRS()}).get();
                        }
                        <#else>
                        updated${name} =  updateStatement.${name?uncap_first}Store${orm.daoSuffix}.select(${getPrimaryKeysFromModel(name?uncap_first)}).get();
                        </#if>
                    }
                    }
                    return updated${name};
