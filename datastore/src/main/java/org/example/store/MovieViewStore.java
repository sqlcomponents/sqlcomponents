...
import org.example.model.MovieView;
    ...
    /**
    * Datastore for the table - movie_view.
    */
    public final class MovieViewStore  {

    /**
    * Retrieves an instance of MovieViewStore.
    *
    * @param theDataManager  The DataManager instance.
    * @param theObserver     The observer to notify for data changes.
    * @return an instance of MovieViewStore
    */
    public static MovieViewStore getMovieViewStore(
    final DataManager theDataManager
    ,final DataManager.Observer theObserver

) {
    return new MovieViewStore(
    theDataManager
    ,theObserver
    ...
    /**
    * Constructor for MovieViewStore.
    *
    * @param theDataManager   The DataManager instance.
    * @param theObserver      The observer for data changes.
    */
    private MovieViewStore(
    final DataManager theDataManager
    ,final DataManager.Observer theObserver
...
public sealed class SelectStatement implements DataManager.Sql<List<MovieView>> permits SelectStatementWithWhere {
        ...
        @Override
        public final List<MovieView> execute(final Connection connection) throws SQLException 
 {
            
		final String query = "SELECT id,title,directed_by FROM movie_view" 
                ...
                return dataManager.sql(query).queryForList(MovieViewStore.this::rowMapper).execute(connection);
	}

        public final int count(final DataSource dataSource) throws SQLException {
		final String query = "SELECT COUNT(*) FROM movie_view" 
                + ( this.whereClause == null ? "" : (" WHERE " + this.whereClause.asSql()) );
        ...
        public final class LimitClause  {
                ...
                public DataManager.Page<MovieView> execute(final DataSource dataSource) throws SQLException 
 {
...
public final DataManager.SelectQuery<Value<?,?>,MovieView> sql(final String sql) {
            return new DataManager.SelectQuery<>(sql, MovieViewStore.this::rowMapper);
    }
    ...
    * @return A new MovieView object.
    * @throws SQLException if any SQL error occurs.
    */
    private MovieView rowMapper(ResultSet rs) throws SQLException 
 {
