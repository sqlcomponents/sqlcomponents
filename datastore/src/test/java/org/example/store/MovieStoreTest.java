...
class MovieStoreTest {
  ...
  /**
   * Test Plan The number of rows updated should be same as the number of rows satisfying the
   * criteria. The updated rows should have the updated values.
   */
  @Test
  void testPartialUpdate() throws SQLException {
    String updatedDirector = "Sathish";
    int updatedRows = this.movieStore.update().set(directedBy(updatedDirector))
    ...
    Assertions.assertAll("Partial Update",
        () -> Assertions.assertEquals(updatedRows, updatedMovies.size(),
            "Updated rows count is not equal"),
        () -> updatedMovies.forEach(
            movie -> Assertions.assertEquals(updatedDirector, movie.getDirectedBy(),
                "Updated value is not equal")));

  }

  /**
   * Test Plan. The returned object should be non-null The returned object should have the same
   * values as the inserted object. The returned object should have non-null id.
   */
  @Test
  ...
  /**
   * Test Plan. The returned list length should be same as the inserted list length. The objects in
   * the returned list should have the same values as the inserted objects. The objects in the
   * returned list should be in the same order as the inserted objects. The objects in the returned
   * list should have non-null id.
   */
  @Test
