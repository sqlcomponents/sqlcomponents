package com.techatpark.scubebird.scubedao.implementation.ibatis;

import freemarker.template.Configuration;
import freemarker.template.Template;
import freemarker.template.TemplateException;
import com.techatpark.scubebird.scubedao.implementation.OrmImplementation;
import com.techatpark.scubebird.scubedao.model.*;
...
public class IBatisImplementation extends OrmImplementation {
	...
	public void writeImplementation(DaoProject project) {
		...
		for (Entity entity : orm.getEntities()) {
			try {
				...
				writeSqlMapImplementation(processedEntity, project
						.getSrcFolder());
			} catch (IOException e) {
	...
	private void writeDaoImplementation(ProcessedEntity entity, String srcFolder,String daoSuffix)
		...
		processTemplates(entity, packageFolder + File.separator + "SqlMap"
				+ entity.getName() + "Dao"  + daoSuffix.trim() + ".java", daoTemplate);
	}

	private void writeSqlMapImplementation(ProcessedEntity entity,
			String srcFolder) throws IOException, TemplateException {
		String packageFolder = getPackageAsFolder(srcFolder, entity
				.getDaoPackage()
				+ ".sqlmapdao.sqlmap");
		new File(packageFolder).mkdirs();
		processTemplates(entity, packageFolder + File.separator
				+ entity.getName() + ".xml", sqlMapTemplate);
	}
	...
	public IBatisImplementation() {
		freeMarkerConfiguration = new Configuration();
		freeMarkerConfiguration.setClassForTemplateLoading(
				IBatisImplementation.class, "/");

		try {
			daoTemplate = freeMarkerConfiguration
					.getTemplate("template/dao/java/ibatis/sqlmapdao.ftl");
	...
	private Template sqlMapConfigTemplate;
